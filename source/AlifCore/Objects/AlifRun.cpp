...
AlifIntT alifRun_simpleFileObject(FILE* _fp, AlifObject* _filename,
	...
	//		fprintf(stderr, "alif: failed to set __الرئيسية__.__محمل__\n");
	//		ret = -1;
	//		fclose(alifcFP);
	//		goto done;
	//	}
...
static AlifIntT printException_message(ExceptionPrintContext* ctx, AlifObject* type,
	...
	AlifObject* modulename = alifObject_getAttr(type, &ALIF_STR(__module__));
	if (modulename == nullptr or !ALIFUSTR_CHECK(modulename)) {
		ALIF_XDECREF(modulename);
		alifErr_clear();
		if (alifFile_writeString("<غير معروف>.", f) < 0) {
			return -1;
		}
	}
	else {
	...
	if (s == nullptr) {
		alifErr_clear();
		if (alifFile_writeString(": <فشل الخلل نص()>", f) < 0) {
			return -1;
		}
	}
	else {
...
static AlifObject* runEval_codeObj(AlifThread* _thread, AlifCodeObject* _co,
	...
	if (!_globals or !ALIFDICT_CHECK(_globals)) {
		alifErr_setString(_alifExcSystemError_, "متغير globals يجب أن يكون من نوع فهرس");
		return nullptr;
	}
